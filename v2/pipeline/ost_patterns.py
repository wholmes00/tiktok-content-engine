...
def build_ost_constraint_prompt(content_angles=None):
    ...
    lines.append("-" * 50)
    lines.append("MANDATORY RULES FOR OST GENERATION:")
    lines.append("")
    lines.append("1. Every video MUST have OST at three positions:")
    lines.append("   - HOOK (0:00): Reinforces the scroll-stop moment")
    lines.append("   - NARRATIVE (mid): 1-3 text cards that guide the story")
    lines.append("   - CTA (end): Drives the viewer to act")
    lines.append("")
    lines.append("2. Hook-position OST should be PERSISTENT when possible")
    lines.append("   (persistent = 60% higher engagement on average)")
    lines.append("")
    ...
    lines.append("")
    lines.append("4. Use emojis strategically — they boost engagement in OST")
    lines.append("   but limit to 1-3 per text card. Don't overdo it.")
    lines.append("")
    lines.append("5. CAPS for emphasis on 1-2 key words max. Full caps only")
    lines.append("   for very short phrases (≤4 words).")
    lines.append("")
    lines.append("6. Each OST card must declare which template it follows")
    lines.append("   (e.g., OST1, OST2) or 'ORIGINAL'.")
    lines.append("")
